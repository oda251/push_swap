...
/*   Updated: 2023/10/29 04:46:54 by yoda             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ms_sub(t_stack *a, t_stack *b, int size, int part_size)
{
	int	i;
	int	j;
	...
	while (i < PARTS)
	{
		...
		{
			if (a->nums[0] < part_size * i + part_size / 2)
				pb(a, b);
			else if (a->nums[0] < part_size * (i + 1) || i == PARTS - 1)
			{
				pb(a, b);
				rb(a, b);
			}
...
static void	move_split(t_stack *a, t_stack *b)
{
	int	size;
	int	part_size;
