...
num_size = int(input("Enter the number of numbers to sort: "))
...
class PsGui:
    def __init__(self, master):
        ...
        nums = random.sample(range(num_size), num_size)
        nums_str = [str(v) for v in nums]
        self.pile_a = nums
        self.first_pile = self.pile_a[:]
        self.pile_b = []
        self.cmds = subprocess.check_output([PUSHS_PATH] + nums_str, stderr=subprocess.STDOUT,
